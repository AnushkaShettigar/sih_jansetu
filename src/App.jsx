...
import { ArrowUpRight, Building2, Check, ChevronDown, CloudSun, Droplets, ImagePlus, Landmark, Lightbulb, Menu, MapPin, Minus, Moon, MoveUpRight, Route, Search, Send, ShieldCheck, Sun, Trash2, Waves, X } from 'lucide-react'
import './App.css'
...
function Navbar() {
  ...
  return <header className="navbar-wrap"><nav className="navbar" aria-label="Main navigation"><a className="nav-brand" href="#top" aria-label="JanSetu home">JanSetu</a><div className={`nav-links ${open ? 'is-open' : ''}`}>{links.map(([label, href]) => <a key={label} href={href} onClick={() => setOpen(false)}>{label}</a>)}</div><div className="gov-mark"><Landmark size={16} /><span>Government of<br /><b>Jharkhand</b></span></div><button className="icon-button menu-button" type="button" aria-label="Toggle navigation" aria-expanded={open} onClick={() => setOpen(!open)}>{open ? <X size={22} /> : <Menu size={22} />}</button></nav></header>
}
function LiveInfo() {
...
function Hero() { return <section className="hero-section" id="top"><div className="hero-orbit orbit-one" /><div className="hero-orbit orbit-two" /><div className="hero-copy"><motion.div initial="hidden" animate="visible" variants={fadeUp} className="eyebrow"><span /> Digital public service, made human</motion.div><motion.h1 initial="hidden" animate="visible" variants={fadeUp}>A better Jharkhand,<br /><em>together.</em></motion.h1><motion.p initial="hidden" animate="visible" variants={fadeUp}>JanSetu connects citizens with government services, making it simple to report civic problems, track complaints and improve the communities we call home.</motion.p><motion.div initial="hidden" animate="visible" variants={fadeUp} className="hero-actions"><a className="button button-primary" href="#report">Report an issue <ArrowUpRight size={17} /></a><a className="button button-secondary" href="#track">Track complaint <Search size={16} /></a></motion.div><div className="hero-trust"><ShieldCheck size={16} /> A transparent bridge between citizens and civic teams</div></div><LiveInfo /></section> }
